termux_step_get_dependencies() {
	...
	while read -r PKG PKG_DIR; do
		...
		if [[ "$TERMUX_INSTALL_DEPS" == "true" || "$cyclic_dependence" = "true" ]]; then
			...
			local local_pkg_file=""
			if [[ "$TERMUX_REPO_PKG_FORMAT" == "debian" ]]; then
				local_pkg_file="${TERMUX_OUTPUT_DIR}/${PKG}_${DEP_VERSION}_${DEP_ARCH}.deb"
			elif [[ "$TERMUX_REPO_PKG_FORMAT" == "pacman" ]]; then
				local_pkg_file="${TERMUX_OUTPUT_DIR}/${PKG}-${DEP_VERSION_PAC}-${DEP_ARCH}.pkg.tar.xz"
			fi
			...
			if [[ -f "$local_pkg_file" && "$TERMUX_FORCE_BUILD_DEPENDENCIES" != "true" ]]; then
				[[ "$TERMUX_QUIET_BUILD" != "true" ]] && echo "Found locally built dependency $PKG@$DEP_VERSION in $TERMUX_OUTPUT_DIR, extracting instead of building..."
				
				if [[ "$TERMUX_ON_DEVICE_BUILD" == "false" ]]; then
					(
						cd "$TERMUX_OUTPUT_DIR"
						if [[ "$TERMUX_REPO_PKG_FORMAT" == "debian" ]]; then
							ar p "${PKG}_${DEP_VERSION}_${DEP_ARCH}.deb" "data.tar.xz" | \
								tar xJ --no-overwrite-dir --transform='s#^.$#data#' -C /
						elif [[ "$TERMUX_REPO_PKG_FORMAT" == "pacman" ]]; then
