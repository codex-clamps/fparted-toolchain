...
7. **Required binary execute bits** — every required regular-file binary under `usr/bin/` or `bin/` is owner-executable with no group/other execute bits (`(mode & 0o111) == 0o100`) in `rootfs-manifest.json` (symlink entries exempt)
